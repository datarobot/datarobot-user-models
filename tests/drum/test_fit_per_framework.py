...
class TestFit:
    ...
    def test_fit_for_use_output_and_nested(
        self, resources, framework, problem, weights, use_output, tmp_path, nested, framework_env
    ):
    ...
    def test_fit(self, resources, framework, problem, docker, weights, tmp_path, framework_env):
        skip_if_framework_not_in_env(framework, framework_env)
    ...
    def test_transform_fit_fails_default(
        self, resources, framework, language, problem, weights, tmp_path, strict, framework_env
    ):
        skip_if_framework_not_in_env(framework, framework_env)
    ...
    def test_transform_fit_disallow_y_output(
        self, resources, tmp_path, framework, language, problem, framework_env
    ):
        skip_if_framework_not_in_env(framework, framework_env)
        input_dataset = resources.datasets(framework, problem)
        target_type = TRANSFORM
    ...
    def test_sparse_transform_fit_disallow_y_output(
        self, framework, resources, tmp_path, framework_env
    ):
        skip_if_framework_not_in_env(framework, framework_env)
    ...
    def test_sparse_transform_fit(self, framework, resources, tmp_path, framework_env):
        skip_if_framework_not_in_env(framework, framework_env)
        input_dataset = resources.datasets(None, SPARSE)
        target_dataset = resources.datasets(None, SPARSE_TARGET)
    ...
    def _create_fit_input_data_dir(
        ...
        if is_sparse:
            ...
            with open(X_file, "w+") as fp:
                ...
                feature_df.to_csv(fp, index=False, lineterminator="\r\n")
        ...
        if problem != ANOMALY:
            ...
            if not is_sparse:
                with open(target_file, "w+") as fp:
                    ...
                    target_series.to_csv(fp, index=False, header="Target", lineterminator="\r\n")
            if is_sparse:
    ...
    def test_duplicate_target_name(self, resources, tmp_path, framework_env):
        ...
        target_dataset = resources.datasets(framework, TARGET_NAME_DUPLICATED_Y)
    ...
    def test_fit_schema_validation(self, resources, tmp_path, framework_env):
    ...
    def test_fit_fails_code_dir_is_output_dir(self, resources, tmp_path, framework_env):
        framework = SKLEARN_REGRESSION
        skip_if_framework_not_in_env(framework, framework_env)
        custom_model_dir = _create_custom_model_dir(
            resources,
    ...
    def test_fit_with_fit_metadata(
        self, resources, tmp_path, skip_predict, output_fit_metadata, framework_env
    ):
        framework = SKLEARN_REGRESSION
        skip_if_framework_not_in_env(framework, framework_env)
        custom_model_dir = _create_custom_model_dir(
        ...
        if skip_predict:
            ...
            assert 180 > data["fit_memory_usage"] > 100
