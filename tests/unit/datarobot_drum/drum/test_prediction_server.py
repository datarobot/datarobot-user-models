...
from datarobot_drum.resource.components.Python.prediction_server.prediction_server import PredictionServer


def create_completion(message_content):
    return ChatCompletion(id="id",
...
def create_completion_chunks(messages):
    def create_chunk(content, finish_reason=None, role=None):
        return ChatCompletionChunk(id="id", choices=[
            chat_completion_chunk.Choice(delta=ChoiceDelta(content=content, role=role), finish_reason=finish_reason,
                                         index=0)], created=0, model='model',
...
class ChatPythonModelAdapter(PythonModelAdapter):
    ...
    def load_model_from_artifact(self, user_secrets_mount_path: Optional[str] = None,
                                 user_secrets_prefix: Optional[str] = None, skip_predictor_lookup=False):
        return ""
...
@pytest.fixture
def test_flask_app():
    with (patch("datarobot_drum.drum.server._create_flask_app") as mock_create_flask_app,
          patch(
