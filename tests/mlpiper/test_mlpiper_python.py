...
class TestPythonEngine:
    ...
    def test_component_argument_building_with_sagemaker(self):
        ...
        comps_desc_list = ComponentsDesc(
            python_engine, pipeline=pipeline, comp_root_path=COMPONENTS_ROOT
        ).load()
        dag = Dag(pipeline, comps_desc_list, python_engine)

        dag_node = dag.get_dag_node(0)
        input_args = dag_node.input_arguments(
            systemConfig, ee_config, comp_only_args=False
        )
        assert input_args["arg1"] == "arg1-value"
        assert (
            input_args["configs"]["engConfig"]["arguments"]["iam_role"]["value"]
            == iam_role_value
        )
