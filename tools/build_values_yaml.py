...
FIELD_ORDER = [
    ...
    FieldSpec("env_name", "", "name", True),
    FieldSpec("id", "", "id", True),
    ...
    # Moved the version fields to the end
    FieldSpec("version_id", "", "environmentVersionId", True),
    FieldSpec("version_label", "", "label", True),
    FieldSpec("version_description", "", "environmentVersionDescription", True),
]
...
def quoted_presenter(dumper, data):
    # Always use double quotes for strings, including multiline ones
...
QuotingDumper.add_representer(OrderedDict, QuotingDumper.represent_dict)
...
def reorder_and_fill_fields(source_data):
    ...
    env_version_id = parse_value(
        source_data.get("environmentVersionId", ""), "version_id"
    )
