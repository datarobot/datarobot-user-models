...
from datarobot_drum.resource.components.Python.uwsgi_component.ml_engine.rest_model_serving_engine import (
...
from datarobot_drum.resource.components.Python.uwsgi_component.restful.nginx_broker import (
    NginxBroker,
)
...
class RESTfulComponent(ConnectableComponent):
        ...
        if engine is not None:
            if (
                ...
                err_msg = (
                    ...
                    "`pip install uwsgi` or `pip install drum[uwsgi]`"
                )
                raise UwsgiNginxCoreException(err_msg)
    ...
    def _setup(self, pipeline_name, monitor_info):
        ...
        uwsgi_entry_point_conf = {
            ...
            ComponentConstants.UWSGI_DISABLE_LOGGING_KEY: str2bool(
                self._params.get(
        ...
        if not self._dry_run and monitor_info[UwsgiConstants.MONITOR_THREAD_KEY]:
            try:
                ...
                if monitor_info[UwsgiConstants.MONITOR_ERROR_KEY]:
                    ...
                    raise UwsgiNginxCoreException(monitor_info[UwsgiConstants.MONITOR_ERROR_KEY])
