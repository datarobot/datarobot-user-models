...
from object_detection_utils import get_submodules_from_kwargs
...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix="", freeze_bn=False):
    ...
    Dropout = get_dropout(backend=backend, layers=layers, models=models, utils=keras_utils)
    ...
    if has_se:
        ...
        target_shape = (
            (1, 1, filters) if backend.image_data_format() == "channels_last" else (filters, 1, 1)
        )
