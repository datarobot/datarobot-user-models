...
def create_image_version(session, endpoint, image_id, env_version, file_folder):
    ...
    payload = {
        'baseEnvironmentVersionId': env_version,
...
def prepare_token(parameter_token):
    ...
    if not token:
        raise ValueError('You need to set DR API token through parameters or DATAROBOT_API_TOKEN env variable')
    return token
...
@click.command()
@click.option('-e', '--base-env', required=True, type=str, help="Name or ID for execution environment")
@click.option('-p', '--path', required=True, type=click.Path(exists=True, file_okay=False, resolve_path=True), help="Path to folder with files that should be uploaded")
@click.option('-n', '--name', default="CustomApp", type=str, help="Name for new custom application. Default: CustomApp")
